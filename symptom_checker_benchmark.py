# CODE 2.4: Final Robust Benchmarking (Retraining LR for consistency)
...
try:
    ...
    # Robust Drop Logic for 'Unnamed: 133'
    ...
    # Load numerical target (y_test is critical for comparison)
...
# --- 2. Data Preparation (Cleaned Features for ALL models) ---
# This ensures ALL models (LR, GNB, SVM) are trained and tested on the identical, clean feature set.
X_orig = df_train.drop('prognosis', axis=1)
X_test_orig = df_test.drop('prognosis', axis=1)

# Create the cleaned symptom list
SYMPTOM_LIST_CLEAN = [col.strip().replace(' ', '_').lower() for col in X_orig.columns]

# Create new DataFrames with cleaned feature names for all models
...
# --- 3a. Baseline: Logistic Regression (Retrained on CLEANED names) ---
# We retrain LR here to ensure it uses the clean feature names, solving the ValueError.
...
joblib.dump(lr_model, 'lr_model_clean.pkl') # Save the new, clean version
print(f"   LR Accuracy: {accuracy_lr*100:.2f}%")


# --- 3b. Gaussian Naive Bayes (GNB) - Trained on CLEANED names ---
...
# --- 3c. Support Vector Machine (SVM) - Trained on CLEANED names ---
...
# --- 4. Benchmarking and Final Model Selection ---
print("\n==================================================================")
